...
class SequenceRunner(QObject):
    ...
    def run_online(
        ...
        frames = [
            (group_command(group, compact=True), self._group_duration(group))
    ...
    def run_online_blocking(
        ...
        selected = groups[start : end + 1]
        commands = [
            group_command(group, compact=True)
            for _ in range(count)
            for group in selected
        ]
        self._session_id += 1
    ...
    def _group_duration(group: ActionGroup) -> int:
        # Ignore download padding slots so timing matches the compact frame.
        return max(
            (
                time_ms
                for _, pwm, time_ms in group.moves
                if not (pwm == 0 and time_ms == 0)
            ),
            default=0,
        )
