...
Gimbal Studio 是面向 ZL-IS2 云台／舵机控制器的跨平台串口图形工具，支持
PWM 云台控制、动作组编辑与执行、工程 INI 读写、下载和串口日志。
...
运行依赖为 PySide6 和 pyserial；测试与打包还需要 pytest 和 PyInstaller。

## 安装

在本目录 `tools/gimbal_studio` 下创建虚拟环境并安装：
...
当前协议常量位于 `src/gimbal_studio/protocol/commands.py`：

- 设置开机动作：`$PTL:<起始组>-<结束组>,<次数>!`
- 取消开机动作：`$PTC!`

这两个字符串以及下载帧需要按控制板型号和固件用真机核对。若与手册或抓包
结果不符，只应校正协议构造器或下载实现，并补充对应测试。
